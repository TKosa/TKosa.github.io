  ...
  effect(tank) {
    this.draw_mirror = function () {
      ...
      ctx.translate(
        ...
        (canvas.height * 4) / 5 - (this.y + this.height / 2)
      );
...
class ShinraTenseiPowerup extends Powerup {
  ...
  static repelTank(repeler, repelee) {
    ...
    repelee.tryMovingTo([
      repelee.x + (adjacent / hypoteneuse) * strength_of_push,
      repelee.y - (opposite / hypoteneuse) * strength_of_push,
    ]);
  }
  ...
  static repelBullet(tank, bullet) {
    ...
    if (new_x_speed ** 2 + new_y_speed ** 2 > game.bullet_speed_limit_squared) {
      return;
    }
