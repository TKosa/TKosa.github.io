...
export class Bullet{
    ...
    handleTankCollisions(){
        ...
        {
            if (tank==this.tank && this.tank.maze.settings.friendly_fire==false){return;}
            if(tank.is_dead==false)
                ...
                {   
                    if (tank == this.tank && (this.tank.maze.tick - this.time_created) < 10) { return }// Prevent self-hit immediately after firing   if (tank == this.tank && (this.tank.maze.tick - this.time_created) < 10) { return }// Prevent self-hit immediately after firing
                    debugger
                    tank.onBulletHit();
                }
            }
